...
func trace_on()->bool:
	return DataManager.is_developer()
...
func next_behavior_actor_attacker(wv:War_Vstate)->int:
	...
	var primary = -1
	var secondary = -1
	var minClearDistance = 999
	var minAssaultDistance = 999
	for wa in candidates:
		map.aStar.update_map_for_actor(wa)
		var route = map.aStar.get_clear_path(wa.position, mainCityLocation)
		if route.empty():
			if primary < 0:
				route = map.aStar.get_assault_path(wa.position, mainCityLocation)
				if route.empty():
					continue
				if route.size() < minAssaultDistance:
					minAssaultDistance = route.size()
					secondary = wa.actorId
		elif route.size() < minClearDistance:
			minClearDistance = route.size()
			primary = wa.actorId
	return secondary if primary < 0 else primary
