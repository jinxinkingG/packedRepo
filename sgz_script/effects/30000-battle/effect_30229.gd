...
func effect_30229_AI_2():
	var changed = _chance_to_trans_enemy_unit()
	if changed <= 0:
		goto_step("3")
		return
	var msg = "{0}部心胆俱裂\n{1}单位变为步兵".format([
...
func _chance_to_trans_enemy_unit()->int:
	var changed = 0
	...
	changed = ske.battle_change_units_type(enemy.actorId, affected, "步")
	ske.battle_report()
	return changed
