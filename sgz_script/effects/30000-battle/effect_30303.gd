...
func effect_30303_AI_start()->void:
	goto_step("start")
	return

func effect_30303_start()->void:
	var msg = "催阵向前，团团围困\n看{0}如何施展！".format([
...
func effect_30303_confirmed()->void:
	ske.battle_cd(99999)

	var msg = "{0}的武器栏被禁用".format([enemy.get_name()])
