...
#纳贿锁定效果
#【纳贿】大战场，诱发技。你方其他武将受到计策伤害的场合，可以发动：对方需选择，是否将100～500金交给你，若对方给金，则你的永久标记[金]+该金数，且该回合内，你周围6格内的你方武将被用计时，施计方命中率+X（X=交给你的金/25）；否则，你恢复该队友本次计策伤害的士兵数。每个回合限1次。
...
func on_trigger_20017()->bool:
	...
	if se.targetId < 0 or se.targetId == actorId:
		return false
