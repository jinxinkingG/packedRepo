...
#雷击主动技 #道术
#【雷击】大战场，主动技。①你可以消耗8机动力，指定一个对方武将，你对目标周围2格距离内的所有非城地形敌人造成道术雷击伤害，并有40%概率，附加定止1回合状态。每个回合限1次。）（注：伤兵量=A*(30~35)/10。其中A=用计者智力-受计者智力，且最小为5。②你为张角，则额外附加<落雷>。

const EFFECT_ID = 20006
...
func effect_20006_AI_2():
	_perform_skill(true, 3001)
	return

func effect_20006_AI_3():
	var se = DataManager.get_current_stratagem_execution()
	report_stratagem_result_message(se, 3001)
	return

func effect_20006_start():
	var se = DataManager.new_stratagem_execution(me.actorId, STRATAGEM)
