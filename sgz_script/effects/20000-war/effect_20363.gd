...
#影咒主动技
#【影咒】大战场，主动技。你可选择1个五行为“土”的其他武将发动。你与目标各自选择1个技能交换给对方，直到本回合结束。<影咒>不可交换，每个回合限1次。附加锁定效果：双方每回合五行的刷新时，刷新种类中加入“土”属性。
...
func effect_20363_start():
	...
	for targetId in get_teammate_targets(me):
		...
		if wa.five_phases != War_Character.FivePhases_Enum.Earth:
			continue
		if SkillHelper.get_actor_skill_names(wa.actorId).empty():
			continue
	...
	for targetId in get_enemy_targets(me):
		...
		if wa.five_phases != War_Character.FivePhases_Enum.Earth:
			continue
		var targetSkills = SkillHelper.get_actor_skill_names(wa.actorId)
...
func effect_20363_2():
	...
	var skills = SkillHelper.get_actor_skill_names(targetId)
	skills.erase("影咒")
...
func effect_20363_3():
	...
	var skills = SkillHelper.get_actor_skill_names(me.actorId)
	skills.erase("影咒")
...
func effect_20363_4():
	...
	ske.cost_war_cd(1)
	if not ske.ban_war_skill(me.actorId, mySkill, 1):
	...
	if not ske.ban_war_skill(targetId, targetSkill, 1):
		...
		return
	ske.add_war_skill(me.actorId, targetSkill, 1)
	ske.add_war_skill(targetId, mySkill, 1)

	var msg = "换手如换刀\n胜负之所，当在精微"
	report_skill_result_message(ske, 2004, msg, 2, me.actorId)
	return
